/mob/living/basic/attack_hand(mob/living/carbon/human/user, list/modifiers)
	...
	if(LAZYACCESS(modifiers, RIGHT_CLICK))
		...
		if(!Move(get_step(src, shove_dir), shove_dir))
			log_attack(user, "shoved and failed to move", src)
			...
			return TRUE
		log_attack(user, "shoved and pushed", src)
	...
	if(!user.combat_mode)
		...
		log_attack(user, response_harm_simple, src)
		updatehealth()
		return TRUE
