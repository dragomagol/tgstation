			...
			if(do_mob(src, patient, 20)) //Slightly faster than default tend wounds, but does less HPS
				if((get_dist(src, patient) <= 1) && (bot_mode_flags & BOT_MODE_ON) && assess_patient(patient))
					...
					if(bot_cover_flags & BOT_COVER_EMAGGED)
						...
						log_attack(src, patient, "pretended to tend wounds on", "internal tools", "([uppertext(treatment_method)]) (EMAGGED)", list("silicon", "surgery"))
					else
						...
						log_attack(src, patient, "tended the wounds of", "internal tools", "([uppertext(treatment_method)])", list("silicon", "surgery"))
