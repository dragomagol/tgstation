			...
			if(do_mob(src, patient, 20)) //Slightly faster than default tend wounds, but does less HPS
				if((get_dist(src, patient) <= 1) && (bot_mode_flags & BOT_MODE_ON) && assess_patient(patient))
					...
					if(bot_cover_flags & BOT_COVER_EMAGGED)
						...
						log_attack(src, "pretended to tend the wounds of", patient, "internal tools", "([uppertext(treatment_method)]) (EMAGGED)", list("silicon", "surgery"))
					else
						...
						log_attack(src, "tended the wounds of", patient, "internal tools", "([uppertext(treatment_method)])", list("silicon", "surgery"))
