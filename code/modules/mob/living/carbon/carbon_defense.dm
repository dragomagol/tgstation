...
/mob/living/carbon/proc/disarm(mob/living/carbon/target)
	...
	if(target.IsKnockdown() && !target.IsParalyzed()) //KICK HIM IN THE NUTS
		...
		log_attack(src, "kicked", target, details = "onto [target.p_their()] side (paralyzing)")
	...
	if(can_hit_something)
		...
		if(directional_blocked || shove_blocked)
			...
			log_attack(src, "shoved", target, details = "knocking [target.p_them()] down")
			return
	...
	log_attack(src, "shoved", target, details = append_message)
