...
/mob/living/carbon/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	...
	if(iscarbon(hit_atom) && hit_atom != src)
		...
		log_attack(src, "crashed into", victim)
...
/mob/living/carbon/throw_item(atom/target)
	...
	if(thrown_thing)
		...
		if(isliving(thrown_thing))
			...
			if(start_T && end_T)
				log_attack(src, "tossed", thrown_thing, "a grab from tile in [AREACOORD(start_T)] towards tile at [AREACOORD(end_T)]")
		var/power_throw = 0
		...
		log_attack(src, "threw [thrown_thing] at", target, details = "[power_throw ? "really hard" : ""]")
...
/mob/living/carbon/proc/disarm_collision(datum/source, mob/living/carbon/shover, mob/living/carbon/target, shove_blocked)
	...
	log_attack(src, "shoved", target, details = "into [name]")
