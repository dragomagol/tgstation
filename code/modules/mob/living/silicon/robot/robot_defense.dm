...
/mob/living/silicon/robot/attackby(obj/item/W, mob/user, params)
	...
	if(W.tool_behaviour == TOOL_WRENCH && opened && !cell) //Deconstruction. The flashes break from the fall, to prevent this from being a ghetto reset model.
		if(!lockcharge)
	...
	if(istype(W, /obj/item/defibrillator) && user.a_intent == "help")
		if(!opened)
			...
			return
		if(!istype(set_model, /obj/item/robot_model/medical))
	...
	if(istype(W, /obj/item/borg/upgrade/))
		...
		if(!opened)
			to_chat(user, "<span class='warning'>You must access the cyborg's internals!</span>")
			return
		if(!src.set_model && U.require_model)
