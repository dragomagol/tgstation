...
/obj/item/reagent_containers/proc/try_splash(mob/user, atom/target)
	...
	if(isturf(target) && reagents.reagent_list.len && thrown_by)
		log_attack(thrown_by, "splashed (thrown)", target, reagent_text, list("reagents"))
		message_admins("[ADMIN_LOOKUPFLW(thrown_by)] splashed (thrown) [reagent_text] on [target] at [ADMIN_VERBOSEJMP(target)].")

	reagents.expose(target, TOUCH)
	log_attack(user, "splashed", target, reagent_text, tags = list("reagents"))
...
/obj/item/reagent_containers/proc/SplashReagents(atom/target, thrown = FALSE, override_spillable = FALSE)
	...
	if(ismob(target) && target.reagents)
		...
		if(thrown_by)
			log_attack(thrown_by, "splashed", M, R, tags = list("reagents"))
		reagents.expose(target, TOUCH)
		...
		if(isturf(target) && reagents.reagent_list.len && thrown_by)
			log_attack(thrown_by, "splashed (thrown)", target, english_list(reagents.reagent_list))
