...
/atom/movable/screen/robot/module/Click()
	...
	if(R.set_model.type != /obj/item/robot_model)
...
/datum/hud/proc/update_robot_modules_display(mob/viewer)
	...
	if(R.shown_robot_modules && screenmob.hud_used.hud_shown)
		...
		var/display_rows = max(CEILING(length(R.set_model.get_inactive_modules()) / 8, 1),1)
		...
		for(var/atom/movable/A in R.set_model.get_inactive_modules())
		...
		for(var/atom/A in R.set_model.get_inactive_modules())
			//Module is not currently active
			screenmob.client.screen -= A
