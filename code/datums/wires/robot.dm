	...
	switch(wire)
		if(WIRE_AI) // Pulse to pick a new AI.
			if(!R.emagged)
				...
				if(new_ai && (new_ai != R.connected_ai))
					...
					log_silicon(usr, "synced", R, details = "[R.connected_ai ? "from [key_name(R.connected_ai)]": ""] to [key_name(new_ai)]")
					if(R.shell)
		...
		if(WIRE_CAMERA) // Pulse to disable the camera.
			if(!QDELETED(R.builtInCamera) && !R.scrambledcodes)
				...
				log_silicon(usr, "toggled", R, details = "cyborg camera to [R.builtInCamera.status ? "ON" : "OFF"] via pulse")
...
/datum/wires/robot/on_cut(wire, mend)
	...
	switch(wire)
		if(WIRE_AI) // Cut the AI wire to reset AI control.
			if(!mend)
				R.notify_ai(AI_NOTIFICATION_CYBORG_DISCONNECTED)
				log_silicon(usr, R, "cut AI wire on cyborg[R.connected_ai ? " and disconnected from [key_name(R.connected_ai)]": ""]")
				if(R.shell)
					R.undeploy()
