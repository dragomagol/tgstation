	...
	switch(action)
		if("time")
			...
			if(value)
				...
				log_attack(user, "modified the timer by [value/10] seconds for", "cell [id]", details = "remaining time: [time_left(seconds = TRUE)]")
		if("start")
			timer_start()
			investigate_log("[key_name(usr)] has started [id]'s timer of [time_left(seconds = TRUE)] seconds", INVESTIGATE_RECORDS)
			log_attack(user, "started the timer for", "cell [id]", details = "remaining time: [time_left(seconds = TRUE)]")
		if("stop")
			investigate_log("[key_name(usr)] has stopped [id]'s timer of [time_left(seconds = TRUE)] seconds", INVESTIGATE_RECORDS)
			log_attack(user, "stopped the timer for", "cell [id]", details = "remaining time: [time_left(seconds = TRUE)]")
			timer_end(forced = TRUE)
		if("flash")
			investigate_log("[key_name(usr)] has flashed cell [id]", INVESTIGATE_RECORDS)
			log_attack(user, "flashed", "cell [id]")
			for(var/datum/weakref/flash_ref as anything in flashers)
				var/obj/machinery/flasher/flasher = flash_ref.resolve()
				if(!flasher)
					flashers -= flash_ref
		...
		if("preset")
			...
			log_attack(user, "set the timer to [preset_time/10] seconds for", "cell [id]", details = "remaining time: [time_left(seconds = TRUE)]")
			if(timing)
				activation_time = world.time
		else
			. = FALSE
