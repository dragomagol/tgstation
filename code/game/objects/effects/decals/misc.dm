...
/obj/effect/decal/chempuff/proc/check_move(datum/move_loop/source, succeeded)
	...
	for(var/atom/movable/turf_atom in our_turf)
		...
		if(!stream)
			...
			log_attack(user, turf_atom, "sprayed", sprayer, "which had [puff_reagents_string]", tags = list("reagents"))
			if(ismob(turf_atom))
				lifetime -= 1
			continue
		...
		if(isliving(turf_atom))
			...
			log_attack(user, turf_mob, "sprayed", sprayer, "which had [puff_reagents_string]", tags = list("reagents"))
			lifetime -= 1
	...
	if(lifetime >= 0 && (!stream || travelled_max_distance))
		...
		log_attack(user, our_turf, "sprayed", sprayer, "which had [puff_reagents_string]", tags = list("reagents"))
		lifetime -= 1
