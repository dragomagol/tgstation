...
/obj/item/hot_potato/proc/force_onto(mob/living/victim, mob/user)
	...
	if(!victim.put_in_hands(src))
		...
		. = TRUE
	if(.)
		log_attack(user, "forced [src] onto", victim, details = "with explosive variables ([detonate_explosion]-[detonate_dev_range]/[detonate_heavy_range]/[detonate_light_range]/[detonate_flash_range]/[detonate_fire_range]) onto", tags = list("traitor"))
		...
		log_attack(user, "tried to force [src] onto", victim, details = "with explosive variables ([detonate_explosion]-[detonate_dev_range]/[detonate_heavy_range]/[detonate_light_range]/[detonate_flash_range]/[detonate_fire_range]) onto", tags = list("traitor"))
